                                  ...
                                  const sp<IMemory>& dataPtr, void *user)
{
    ...
    if (lcdev->data_timestamp_callback != NULL && lcdev->request_memory != NULL) {
        ...
        if (mem != NULL) {
            ALOGV("%s: Posting data to client timestamp:%lld\n", __FUNCTION__,
                  systemTime());
...
static int camera_device_open(const hw_module_t* module, const char* name, hw_device_t** device)
{
    ...
    lcdev->id = cameraId;
    lcdev->hwif = KobeCameraWrapper::createInstance(cameraId);
    if (lcdev->hwif == NULL) {
        free(camera_ops);
        free(lcdev);
        return -EIO;
